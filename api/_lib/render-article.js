...
import { formatIST, truncateAtWord, hostnameOf } from "./text.js";
...
export function card(post, { eager = false } = {}) {
  ...
  return html`
        <li class="news-card">
          ...
          <p class="news-meta">
            <time datetime="${new Date(post.published_at).toISOString()}">${formatIST(post.published_at)}</time>
...
${crumbs(seo.trail)}
      ...
      <p class="news-meta">
        <time datetime="${new Date(post.published_at).toISOString()}">${formatIST(post.published_at)}</time>
        ${cat ? html`<a href="${hubPath(cat)}">${cat.label}</a>` : ""}
        ${post.user_name ? html`<span>by ${post.user_name}</span>` : ""}
      </p>
      ...
      <div class="pix">
        <ol class="article-slides" tabindex="0" aria-label="Story cards">
          ${slides.map((s, i) => {
            ...
            return html`
          <li><img src="${img.src}" srcset="${img.srcset}" sizes="(max-width: 640px) 80vw, 360px" alt="${title}${slides.length > 1 ? ` — card ${i + 1} of ${slides.length}` : ""}"${raw(i === 0 ? ' loading="eager" fetchpriority="high"' : ' loading="lazy"')} decoding="async" /></li>`;
          })}
        </ol>
        ${slides.length > 1
          ? html`
